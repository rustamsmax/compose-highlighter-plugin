package uz.rsteam.intellij.plugin.color.preview
...
class ComposeColorProvider : ElementColorProvider {
  ...
  override fun getColorFrom(element: PsiElement): AwtColor? {
    return getCallExpression(element)?.let { callExpression ->
      callExpression.getColor()?.toAwtColor()
    }
  }

  private fun getCallExpression(element: PsiElement): KtCallExpression? {
  ...
  override fun setColorTo(element: PsiElement, color: AwtColor) {
    val expression = getCallExpression(element) ?: return
    val currentComposeColor = expression.getColor() ?: return
    val composeColor = ComposeColor(color.red, color.green, color.blue, color.alpha)
    ...
    when (expression.valueArguments.getConstructorType()) {
      ColorConstructors.ULONG -> {
        ...
        val newArgument =
          factory.createNewULongExpression(argument = argument, value = composeColor.value)
  ...
  private fun List<KtValueArgument>.getConstructorType(): ColorConstructors? {
    ...
    val isULong =
      firstArgument.isTyped(KtNodeTypes.INTEGER_CONSTANT) && firstArgumentText.contains("u", true)
    val isInt =
      firstArgument.isTyped(KtNodeTypes.INTEGER_CONSTANT) && firstArgumentText.toIntOrNull() != null
